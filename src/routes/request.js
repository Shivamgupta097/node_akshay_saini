...
requestRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!requestedData) {
        throw new Error(
          "No connection  of interest were found to changed to accepted or reject ",
        );
      }

      requestedData.status = status;

      // console.log("requested data =>"), reques

      const connectionDataWithStatus = await requestedData.save();

      console.log(connectionDataWithStatus, "connectionDataWithStatus");
      ...
      res.status(400).json({ message: error.data.message });
    }
  },
);
